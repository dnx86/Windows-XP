...
# Dell Optiplex 9010 Windows XP drivers pack
# See link below for list of contents:
# https://dl.dell.com/FOLDER02061610M/1/9010-xp-A05-8YR57.html
$pathDelldrivers = "$PSScriptRoot\9010-xp-A05-8YR57.CAB"  # CHANGE HERE!
...
# The latest version of the Windows Update Agent for Windows 7, Windows Vista, and Windows XP is 7.6.7600.256.
# https://support.microsoft.com/en-us/topic/information-for-network-administrators-about-how-to-obtain-the-latest-windows-update-agent-c1ebc7ca-27c3-e6b0-5da9-a1c6ffe12e82
...
$pathWUA = "$PSScriptRoot\windowsupdateagent30-x86.exe"
...
#
# Check if the CUs folder exists and if it has MSU files in it.
# Check if listCUs is not empty.
if ( (Test-Path -Path "$pathCU") -and ((Get-ChildItem -Path "$pathCU" -Filter "*.msu").Count -ne 0) -and ($listCUs.Count -gt 0) ) {
...
if (Test-Path -Path $pathDelldrivers) {
    ...
    if ( (Get-AuthenticodeSignature -FilePath $pathDelldrivers).Status -ne "Valid" ) {
        ...
        # Delete 9010 folder and it's contents if it exists
        # CHANGE ALL INSTANCES OF 9010 IF USING SCRIPT FOR A DIFFERENT DELL SYSTEM!
        if (Test-Path -Path "$PSScriptRoot\9010") {  # CHANGE HERE!
            Remove-Item -Path "$PSScriptRoot\9010" -Recurse -Force  # CHANGE HERE!
        }
        # Extract the drivers
        ...
        Copy-Item -Path "$PSScriptRoot\9010\xp\x86\*" -Destination $tempPath -Recurse -Force  # CHANGE HERE!

        $pathStoragedrivers = "$PSScriptRoot\9010\xp\x86\storage\PM7TD_A01-00\xpx86"  # CHANGE HERE!
        ...
        Remove-Item -Path "$PSScriptRoot\9010" -Recurse -Force
    }
}
...
# Copy Windows Update Agent to pathMount and add an entry for it to run
# in winntSIF.
$groWUA = ""  # For the GuiRunOnce section.
$copyWUA = ""  # Used in the batch script.
if (Test-Path $pathWUA) {
    Write-Host "Copying $pathWUA to $pathMount" -ForegroundColor DarkGreen
    Copy-Item -Path "$pathWUA" -Destination "$pathMount"

    $fileName = (Get-ChildItem -Path "$pathWUA").Name

    $groWUA = "`"%SYSTEMDRIVE%\$fileName /quiet /norestart`""
    $copyWUA = "copy `"X:\$fileName`" `"Z:\$fileName`""
}
...
rem Remove USB flash drive and press enter to reboot!
pause
